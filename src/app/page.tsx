...
import { Search, ArrowRight, Gift, Lock, Star } from 'lucide-react'; // Import icons
import Link from 'next/link'; // Import Link for navigation
import { getFeaturedTools, getAllCategories, getIconComponent, Category, Tool } from '@/lib/data/tools'; // Import data loading functions and types
...
const whyUsFeatures = [
  ...
  // Use a generic CheckCircle icon name string
  { icon: Star, title: 'Curated Tools Only', description: 'Handpicked, high-quality tools.' },
];
        ...
        const fetchData = async () => {
            ...
            try {
                const [tools, categories] = await Promise.all([
                    getFeaturedTools(), // Assuming this fetches featured tools
                    getAllCategories() // Assuming this fetches all categories
                ]);
                setFeaturedTools(tools);
                // Select popular categories (e.g., first 6) after fetching
        ...
        <h2 className="text-3xl font-bold text-center mb-12">Featured Tools</h2>
         {isLoadingTools ? (
            ...
            {featuredTools.map((tool) => {
                // We need category details to display the icon. Fetch categories first, then map.
                // For simplicity here, assuming popularCategories has the necessary info or fetch separately.
                const categoryInfo = popularCategories.find(cat => cat.slug === tool.categorySlug);
                const CategoryIcon = categoryInfo ? getIconComponent(categoryInfo.iconName) : Star; // Default icon

                return (
                <Link key={tool.slug} href={`/tools/${tool.slug}`} className="block h-full group">
                    <Card className="bg-card hover:border-accent transition-colors duration-300 group transform hover:-translate-y-1 hover:shadow-xl cursor-pointer flex flex-col h-full rounded-lg overflow-hidden"> {/* Added overflow-hidden */}
                        {/* Tool Image */}
                        <div className="relative w-full aspect-video overflow-hidden"> {/* Ensure image div is relative and clips */}
                            <Image
                        ...
                        {/* "View Tool" Button - Link handled by parent Link component */}
                         <Button variant="outline" size="sm" className="w-full group-hover:bg-accent group-hover:text-accent-foreground transition-colors duration-300 mt-auto pointer-events-none"> {/* Added pointer-events-none */}
      ...
      <section className="w-full max-w-6xl px-4 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
          {whyUsFeatures.map((feature, index) => {
             const IconComp = feature.icon; // Dynamically get the icon component
             return (
                <div key={index}>
